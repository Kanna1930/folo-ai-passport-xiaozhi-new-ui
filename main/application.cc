...
void Application::Run() {
    ...
    while (true) {
        ...
        if (bits & MAIN_EVENT_CLOCK_TICK) {
            ...
            // Screen-only power saving: keep Wi-Fi and wake-word detection alive,
            // but turn the LCD backlight off after 3 minutes of idle time.
            if (GetDeviceState() == kDeviceStateIdle) {
                if (screen_idle_seconds_ < kScreenOffTimeoutSeconds) {
                    ++screen_idle_seconds_;
                }
                if (!screen_is_off_ && screen_idle_seconds_ >= kScreenOffTimeoutSeconds) {
                    if (auto backlight = board.GetBacklight()) {
                        backlight->SetBrightness(0);
                        screen_is_off_ = true;
                        ESP_LOGI(TAG, "Screen backlight off after %u seconds idle",
                                 kScreenOffTimeoutSeconds);
                    }
                }
            } else {
                screen_idle_seconds_ = 0;
            }
...
void Application::NotifyUserActivity() {
    screen_idle_seconds_ = 0;
    if (screen_is_off_) {
        ...
        ESP_LOGI(TAG, "Screen backlight restored by activity");
    }
}
