...
void Backlight::SetBrightness(uint8_t brightness, bool permanent) {
    ...
    // A wake during the fade to zero must cancel the old target, even if the
    // current brightness still happens to equal the requested wake brightness.
    if (target_brightness_ == brightness) {
        return;
    }
