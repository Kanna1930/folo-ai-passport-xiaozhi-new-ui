...
void LcdDisplay::SetupUI() {
    ...
    lv_obj_set_style_text_font(battery_percent_label_, lvgl_theme->text_font()->font(), 0);
...
void LcdDisplay::SetupUI() {
    ...
    // Large portrait zone for the low-resolution pixel assistant.
    lv_obj_set_size(emoji_box_, 200, 200);
...
void LcdDisplay::SetEmotion(const char* emotion) {
    ...
    if (image->IsGif()) {
        ...
        // Scale our 176x176 state portraits up to fill most of the 240px-wide LCD.
        if (strcmp(emotion, "neutral") == 0 || strcmp(emotion, "listening") == 0 ||
            strcmp(emotion, "thinking") == 0 || strcmp(emotion, "speaking") == 0) {
            const auto* dsc = image->image_dsc();
            if (dsc != nullptr && dsc->header.w > 0 && dsc->header.h > 0) {
                constexpr int kPortraitSize = 188;
                int scale_w = 256 * kPortraitSize / dsc->header.w;
                int scale_h = 256 * kPortraitSize / dsc->header.h;
                lv_image_set_scale(emoji_image_, scale_w < scale_h ? scale_w : scale_h);
            }
        } else {
