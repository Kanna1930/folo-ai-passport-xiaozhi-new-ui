...
def get_emoji_collection_path(default_emoji_collection, noto_fonts_path, project_root=None):
    ...
    # Folo AI Passport custom low-resolution pixel assistant collection.
    # Kept inside this repository so phone/GitHub cloud builds do not need extra downloads.
    if default_emoji_collection == 'folo-pixel-sister':
        if project_root:
            custom_path = os.path.join(project_root, 'main', 'assets', 'folo-pixel-sister')
            if os.path.exists(custom_path):
                return custom_path
            print(f"Warning: Folo pixel assistant collection not found: {custom_path}")
        return None
